...
- [x] **BUG-12 — Spiral still cropped on mobile: bounds fit the socket points, but the drawn curve sweeps past them** | Priority: P1 | S | Reported: 2026-08-06 (player, mobile, with iPhone screenshots) | Completed: 2026-08-06 | Owner: @claude | Branch: `fix/spiral-curve-bounds` → `main` @ `v0.9.10`
  *Follows BUG-10 (v0.9.8).* A player follow-up — this time with **iPhone screenshots** — showed the spiral **still clipped at the screen sides**. v0.9.8 was right in principle and incomplete in practice: it extended the content bounds to cover every **discrete socket point**, but the spiral is **drawn as a continuous curve** that sweeps through **all the angles between** those sockets. Its true extremes therefore lie **outside the socket bounding box**, and the gap is **worst on sparse levels** — level **II** especially, where few sockets mean long unsampled arcs, and the **arc and edge emitters** were the pieces cropped at the screen edges.

  **Fix — measure what is actually drawn:**
  - Bounds are now computed by **densely sampling the continuous curve exactly as the renderer draws it**, so the fit covers the real swept extent rather than an under-sampled approximation of it.
  - **Compact padding raised 34 → 44 world units**, so content never sits flush against the bezel even at the exact fit.

  **Verified:**
  - **Dense curve sampling fully on-screen across levels 2 / 4 / 6 / 12 / 18**, with **all component rings safe**.
  - **Zero console errors**; solvability harness green.

  **Deploy: deploying** — merged to `main` and tagged `v0.9.10`; awaiting the "Request Pages build" run and published-commit confirmation (see OPS-4).
